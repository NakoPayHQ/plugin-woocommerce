...
JS;
    ...
    public function receipt_page($order_id): void
    {
        ...
        if (!$row) {
            ...
            NakoPay_Orders::save([
                ...
                'payment_uri'        => (string) ($resp['bip21'] ?? $resp['payment_uri'] ?? ''),
            ]);
            $row = NakoPay_Orders::find_open_for_order((int) $order_id);
        }
    ...
    private function handle_webhook(): void
    {
        ...
        if (str_starts_with($event_type, 'subscription.')) {
            $handled = NakoPay_Subscriptions::handle_event($event_type, $event['data']);
            status_header($handled ? 200 : 400);
            echo $handled ? 'ok' : 'unhandled subscription event';
            exit;
        }

        // Invoice events (existing flow)
