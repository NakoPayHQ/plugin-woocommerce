...
* Subscription webhook handling (created, renewed, past_due, canceled, updated)
* WooCommerce Subscriptions integration (auto status sync)
...
NakoPay fires the following subscription webhook events to your WooCommerce webhook URL:

* `subscription.created` - New subscription activated
* `subscription.renewed` - Renewal invoice generated, period advanced
* `subscription.past_due` - Payment overdue (3-day grace period expired)
* `subscription.updated` - Subscription details changed
* `subscription.canceled` - Subscription ended (manual or non-payment)

If WooCommerce Subscriptions is installed, the plugin automatically maps these to WC subscription statuses (active, on-hold, cancelled). Without WC Subscriptions, events are logged as order notes.
